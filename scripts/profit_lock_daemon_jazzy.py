...
Forked from profit_lock_daemon.py for JazzyHazzy R1 paper account (PA38IUKNR237).
...
Account lock: PA38IUKNR237 (JazzyHazzy R1 paper). Will refuse to run on any other account.
...
LOCK_STATE = STATE_DIR / "profit_lock_jazzy_state.json"
LOG_FILE = Path.home() / ".openclaw" / "workspace" / "memory" / "profit_lock_daemon_jazzy.jsonl"
LOG_FILE.parent.mkdir(parents=True, exist_ok=True)

POLL_INTERVAL_SEC = 60
ALPACA_BASE = "https://paper-api.alpaca.markets/v2"
EXPECTED_ACCOUNT = "PA38IUKNR237"
...
def assert_account():
    ...
    if actual != EXPECTED_ACCOUNT:
        log("fatal_account", f"Expected JazzyHazzy ({EXPECTED_ACCOUNT}) but got {actual}. Refusing to run.")
        raise SystemExit(2)
    eq = float(r.json().get("equity", 0))
    log("startup", f"Confirmed JazzyHazzy {actual} equity ${eq:,.0f}")
