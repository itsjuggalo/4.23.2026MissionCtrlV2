...
def fetch_flow_candidates():
    """
    Read flow_alerts_today and flow2_alerts_today JSON.
    Handles BOTH flat and nested entry structures (some entries have alert sub-dict).
    Returns dict keyed by OCC symbol.
    
    Tier classification:
      T1 huge:    totalFlowValue >= 1M  AND  SWEEPS > 0
    ...
    candidates = {}
    now = int(time.time())
    cutoff = now - 72 * 3600  # 72h cutoff to handle weekend gap
    ...
    for fname in ['flow_alerts_today.json', 'flow2_alerts_today.json']:
        ...
        for entry in items:
            ...
            try:
                ...
                expiry_str = a.get('Expiry') or a.get('Expiration', '')
                ...
                # Fall back to OptionSymbol if scraper provides one
            ...
            candidates[occ] = {
                'symbol': symbol,
                'strike': strike if 'strike' in dir() else 0,
