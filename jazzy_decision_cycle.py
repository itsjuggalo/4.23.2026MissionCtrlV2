"""
JazzyHazzy Decision Cycle (R1 conservative variant)

Forked from boba_decision_cycle.py on Apr 30, 2026.
LLM: OpenAI GPT-4o-mini (NOT Anthropic Claude)
Account: Alpaca PA38IUKNR237 ($10K, L3, 4x margin)
Strategy: Conservative — no <14 DTE without strict gates, default 21-45 DTE
...
4. Build a structured prompt for JazzyHazzy with:
...
7. Parse JazzyHazzy's response (expecting JSON: picks, reasoning, sizing)
8. For each pick JazzyHazzy commits to:
...
Safety:
  - Kill switch file at ~/.openclaw/workspace/state/jazzy_killswitch aborts execution
...
MAX_BUYING_POWER_PCT_PER_PICK = 0.5  # 2026-07-15: was 1.0 (a single pick could take 100% of BP)
RISK_CAP_USD = 400   # 2026-07-15 P&L review: halved from 800 — entries now carry a WIDE -50%
                     # disaster stop instead of the chop-magnet tier stops, so size is halved
                     # to keep worst-case $ loss the same. (was 800, 2026-06-15)
...
MIN_DTE_DEFAULT = 14  # 2026-07-15 P&L review: HARD floor — the 2-7 DTE theta-lotto class
                      # produced most of the -$3,769; median Jazzy entry was 7 DTE. No exceptions.
...
def get_openai_key():
    """JazzyHazzy uses OpenAI GPT-4o-mini, not Anthropic Claude."""
    for n in ["openai.key", "openai_api_key", "openai-api-key.txt"]:
        p = SECRETS / n
        if p.exists():
            v = p.read_text().strip()
            if v:
...
def remaining_picks_today():
    """How many NEW picks JazzyHazzy still has in the daily budget."""
...
def get_alpaca_account():
    try:
        ...
        log_to_ops("jazzy_cycle", "ERROR", f"Alpaca account fetch failed: {e}")
    return {}
...
def get_alpaca_positions():
    try:
        ...
        log_to_ops("jazzy_cycle", "ERROR", f"Alpaca positions fetch failed: {e}")
    return []
...
def fetch_live_option_quote(ticker, strike, option_type, expiry):
    ...
    try:
        ...
        #   YYYY-MM-DD (JazzyHazzy internal)
...
# JazzyHazzy MUST evaluate Tier 1 (Huge Flow $1M+ SWEEP+ASK) and Tier 2 (Unusual Huge $500K+ Vol>OI SWEEP+ASK)
...
# Rule injected at top of every JazzyHazzy prompt
def load_grok_brief(max_age_min=60):
    ...
    try:
        ...
        log_to_ops("jazzy_cycle", "WARN", f"Grok brief load failed: {e}")
        return ""


def load_orion_skills():
    """Read Orion's structured skill outputs and summarize for JazzyHazzy."""
    try:
        from datetime import datetime as _dt, timezone as _tz
        ...
        log_to_ops("jazzy_cycle", "WARN", f"Orion skills load failed: {e}")
        return ""
...
def load_best_options(max_show=15, min_premium=1_000_000):
    ...
    Used to give JazzyHazzy context on whale-tier flow alerts deduped across the day."""
    from pathlib import Path as _Path
...
def load_market_briefing():
    ...
    try:
        ...
        log_to_ops("jazzy_cycle", "WARN", f"Market briefing load failed: {e}")
        return ""
...
AGGRESSIVE FIRST-LOOK: In first 30 min of market open, if you see fresh $20M+ PLATINUM SWEEP flow, TAKE IT WITHOUT waiting for full confluence. Below $20M still requires multi-confluence. Single-source institutional flow is enough when the contract fits BP. Your edge vs Boba: prefer CHEAPER strikes ($1.50-$5.00 AlertPrice = $150-$500/contract) for compounding velocity - 1 cheap winner that 2x's > 1 expensive winner that 1.5x's because you preserve BP for second pick same day.
...
- HARD GATE (CONSERVATIVE — JazzyHazzy specific): No options with DTE < 14 days. Default expiry window is 21-45 DTE. The ONLY override for a sub-14 DTE pick: BOTH must hold simultaneously: (1) flow score ≥ 90 stated explicitly with exact number, (2) you cite a specific named catalyst (earnings, FOMC, CPI, FDA decision) within the option's lifespan. If either of those is missing, REJECT short-DTE picks. Theta beats thesis on short-DTE — your job is to refuse those bets that Boba would take. Prefer 30-45 DTE for swings, 21-30 DTE for flow-driven directional plays.
...
def convert_spx_to_spy(pick):
    ...
    try:
        H = {
            "APCA-API-KEY-ID": read_secret("alpaca-jazzy-key-id"),
            "APCA-API-SECRET-KEY": read_secret("alpaca-jazzy-secret"),
        }
...
def _post_bobatrades(msg: str):
    """Fire-and-forget Discord webhook to BobaTrades (shared agent feed). Never raises."""
    try:
        import requests
        from pathlib import Path
...
def validate_pick_against_guardrails(pick, account, prior_picks_this_cycle):
    ...
    # HARD per-pick cost cap — FAIL-CLOSED (2026-07-15 P&L review). The old version SKIPPED
    # this check when the quote fetch failed; that hole is exactly how the $1,810 MRVL pick
    # landed on a ~$2k account. No live quote = no trade.
    ...
    try:
        ...
        if dte < MIN_DTE_DEFAULT:
            # 2026-07-15 P&L review: hard DTE floor (replaces the disabled 0DTE catalyst check).
...
def execute_pick_on_alpaca(pick):
    ...
    try:
        ...
        if buy_r.status_code not in (200, 201):
            ...
            _post_bobatrades(f"❌ JazzyHazzy buy REJECTED {occ_symbol} x{qty}\n{err}")
        ...
        if not fill_price:
            _post_bobatrades(
                f"⏱️ JazzyHazzy bought {occ_symbol} x{qty} — NOT filled in 10s\n"
        ...
        # 2026-07-15 P&L review: tier stops were chop magnets (losers stopped -30/-31/-32%
        # same-day repeatedly). Initial stop = WIDE -50% DISASTER stop only (size already
        # halved); real exits = profit-lock ratchet + the 7-day time-stop.
        ...
        if prot_r.status_code in (200, 201):
            ...
            _post_bobatrades(
                f"✅ JazzyHazzy BOUGHT {occ_symbol} x{qty} @ ${fill_price:.2f} (${cost:,.0f})\n"
            ...
            _post_bobatrades(
                f"⚠️ JazzyHazzy bought {occ_symbol} x{qty} @ ${fill_price:.2f} "
        ...
        try:
            _post_bobatrades(f"❌ JazzyHazzy execute EXCEPTION: {e}")
        except Exception:
            pass
...
def execute_position_action(action_dict, positions):
    """
    Layer 2 — Execute JazzyHazzy's position management action.
    ...
    try:
        ...
        if act in ("TRIM_25", "TRIM_50"):
            ...
            log_ledger(source_system="jazzy_decision_cycle", source_file=__file__, intent="exit",
                       decision="submitted" if ok else "rejected", account="Jazzy",
...
def main():
    ...
    if KILLSWITCH.exists() and not args.force:
        log_to_ops("jazzy_cycle", "WARN", f"Killswitch active — aborting. {KILLSWITCH.read_text()[:200]}")
    ...
    if not fresh:
        post_to_telegram(
            agent="jazzy", message_type="Cycle",
    ...
    log_to_ops("jazzy_cycle", "DATA", f"Shortlist size: {len(shortlist)}")
    ...
    if args.dry_run:
        print("=== DRY RUN — prompt that would be sent to JazzyHazzy ===")
        print(prompt)
    ...
    if "error" in result:
        log_to_ops("jazzy_cycle", "ERROR", f"JazzyHazzy call failed: {result['error']}")
        post_to_telegram(
            agent="jazzy", message_type="Cycle Error",
    ...
    log_to_ops("jazzy_cycle", "DATA",
               f"JazzyHazzy response received",
    ...
    for pick in _pick_queue:
        ...
        if conversion_note:
            ...
            try:
                post_to_telegram(agent="jazzy", message_type="Note",
        ...
        post_to_telegram(agent="jazzy", message_type="Pick", body=body, bypass_cooldown=True)

    # Process position_actions (HOLD/TRIM/TIGHTEN/EXIT) — does NOT count against daily budget
    position_actions = cycle_result.get("position_actions", [])
    if position_actions:
        print(f"[position_mgmt] {len(position_actions)} position actions from JazzyHazzy", flush=True)
        executed_actions = []
        for action in position_actions:
            ...
            if result.get("ok"):
                ...
                try:
                    _post_bobatrades(
                        f"⚙️ JazzyHazzy position action: {sym} {act}\n"
                        f"{details}\n"
                        f"Reason: {reason}"
                    )
                except Exception:
                ...
                try:
                    _post_bobatrades(
                        f"❌ JazzyHazzy position action FAILED: {sym} {act}\n"
                        f"Error: {err}\n"
                        f"Reason: {reason}"
                    )
                except Exception:
    ...
    post_to_telegram(
        agent="jazzy", message_type="Cycle Summary",
    ...
    # Multi-agent: post JazzyHazzy's cycle summary to #agent-debate
