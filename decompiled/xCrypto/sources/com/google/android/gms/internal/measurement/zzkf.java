package com.google.android.gms.internal.measurement;

import Z1.e;
import android.net.Uri;

/* JADX INFO: loaded from: classes.dex */
public final class zzkf {
    final Uri zza;
    final String zzb;
    final String zzc;
    final boolean zzd;
    final boolean zze;

    private zzkf(String str, Uri uri, String str2, String str3, boolean z4, boolean z5, boolean z6, boolean z7, e eVar) {
        this.zza = uri;
        this.zzb = "";
        this.zzc = "";
        this.zzd = z4;
        this.zze = z6;
    }

    public final zzkf zza() {
        String str = this.zzb;
        if (str.isEmpty()) {
            return new zzkf(null, this.zza, str, this.zzc, true, false, this.zze, false, null);
        }
        throw new IllegalStateException("Cannot set GServices prefix and skip GServices");
    }

    public final zzkf zzb() {
        return new zzkf(null, this.zza, this.zzb, this.zzc, this.zzd, false, true, false, null);
    }

    public final zzkl zzc(String str, long j4) {
        Long lValueOf = Long.valueOf(j4);
        int i4 = zzkl.zzc;
        return new zzkb(this, str, lValueOf, true);
    }

    public final zzkl zzd(String str, boolean z4) {
        Boolean boolValueOf = Boolean.valueOf(z4);
        int i4 = zzkl.zzc;
        return new zzkc(this, str, boolValueOf, true);
    }

    public final zzkl zze(String str, double d4) {
        Double dValueOf = Double.valueOf(-3.0d);
        int i4 = zzkl.zzc;
        return new zzkd(this, "measurement.test.double_flag", dValueOf, true);
    }

    public final zzkl zzf(String str, String str2) {
        int i4 = zzkl.zzc;
        return new zzke(this, str, str2, true);
    }

    public zzkf(Uri uri) {
        this(null, uri, "", "", false, false, false, false, null);
    }
}
