...
public class QueryEngine {
    ...
    private void createCacheIndexes(Query query, QueryContext queryContext, int i4) {
        ...
        Logger.debug(LOG_TAG, "Query: %s, scans %s local documents and returns %s documents as results.", query.toString(), Integer.valueOf(queryContext.getDocumentReadCount()), Integer.valueOf(i4));
        if (queryContext.getDocumentReadCount() > this.relativeIndexReadCostPerDocument * ((double) i4)) {
    ...
    private boolean needsRefill(Query query, int i4, ImmutableSortedSet<Document> immutableSortedSet, SnapshotVersion snapshotVersion) {
        if (!query.hasLimit()) {
            return false;
        }
    ...
    public void setIndexAutoCreationEnabled(boolean z4) {
        this.indexAutoCreationEnabled = z4;
    }

    public void setIndexAutoCreationMinCollectionSize(int i4) {
        this.indexAutoCreationMinCollectionSize = i4;
    }

    public void setRelativeIndexReadCostPerDocument(double d4) {
        this.relativeIndexReadCostPerDocument = d4;
    }
}
