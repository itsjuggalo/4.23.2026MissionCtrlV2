...
final class MemoryMutationQueue implements MutationQueue {
    ...
    private ImmutableSortedSet<DocumentReference> batchesByDocumentKey = new ImmutableSortedSet<>(Collections.EMPTY_LIST, DocumentReference.BY_KEY);
    private int nextBatchId = 1;
    private AbstractC1062i lastStreamToken = WriteStream.EMPTY_STREAM_TOKEN;
    ...
    private int indexOfBatchId(int i4) {
        if (this.queue.isEmpty()) {
            return 0;
        }
        return i4 - this.queue.get(0).getBatchId();
    }

    private int indexOfExistingBatchId(int i4, String str) {
        int iIndexOfBatchId = indexOfBatchId(i4);
    ...
    public MutationBatch addMutationBatch(Timestamp timestamp, List<Mutation> list, List<Mutation> list2) {
        ...
        if (size > 0) {
            Assert.hardAssert(this.queue.get(size - 1).getBatchId() < i4, "Mutation batchIds must be monotonically increasing order", new Object[0]);
        }
        ...
        for (Mutation mutation : list2) {
            this.batchesByDocumentKey = this.batchesByDocumentKey.insert(new DocumentReference(mutation.getKey(), i4));
    ...
    public List<MutationBatch> getAllMutationBatchesAffectingDocumentKeys(Iterable<DocumentKey> iterable) {
        ImmutableSortedSet<Integer> immutableSortedSet = new ImmutableSortedSet<>(Collections.EMPTY_LIST, Util.comparator());
    ...
    public MutationBatch getNextMutationBatchAfterBatchId(int i4) {
        int iIndexOfBatchId = indexOfBatchId(i4 + 1);
    ...
    @Override // com.google.firebase.firestore.local.MutationQueue
    public MutationBatch lookupMutationBatch(int i4) {
        int iIndexOfBatchId = indexOfBatchId(i4);
        if (iIndexOfBatchId < 0 || iIndexOfBatchId >= this.queue.size()) {
            return null;
        }
        MutationBatch mutationBatch = this.queue.get(iIndexOfBatchId);
        Assert.hardAssert(mutationBatch.getBatchId() == i4, "If found batch must match", new Object[0]);
        return mutationBatch;
    }
