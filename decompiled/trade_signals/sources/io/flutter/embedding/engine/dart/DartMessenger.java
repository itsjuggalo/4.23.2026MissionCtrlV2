...
class DartMessenger implements BinaryMessenger, PlatformMessageHandler {
    ...
    public static class BufferedMessageInfo {
        ...
        public BufferedMessageInfo(ByteBuffer byteBuffer, int i8, long j8) {
    ...
    public static class Reply implements BinaryMessenger.BinaryReply {
        ...
        public Reply(FlutterJNI flutterJNI, int i8) {
    ...
    public static class SerialTaskQueue implements DartMessengerTaskQueue {
        ...
        public void dispatch(Runnable runnable) {
            ...
            this.executor.execute(new Runnable() { // from class: io.flutter.embedding.engine.dart.b
                @Override // java.lang.Runnable
                public final void run() {
                    this.f18964a.lambda$dispatch$0();
                }
            });
        }
    }
    ...
    private void dispatchMessageToQueue(final String str, final HandlerInfo handlerInfo, final ByteBuffer byteBuffer, final int i8, final long j8) {
        DartMessengerTaskQueue dartMessengerTaskQueue = handlerInfo != null ? handlerInfo.taskQueue : null;
        TraceSection.beginAsyncSection("PlatformChannel ScheduleHandler on " + str, i8);
        Runnable runnable = new Runnable() { // from class: io.flutter.embedding.engine.dart.a
            @Override // java.lang.Runnable
            public final void run() {
                this.f18958a.lambda$dispatchMessageToQueue$0(str, i8, handlerInfo, byteBuffer, j8);
            }
        };
    ...
    private void invokeHandler(HandlerInfo handlerInfo, ByteBuffer byteBuffer, int i8) {
        if (handlerInfo != null) {
            try {
                Log.v(TAG, "Deferring to registered handler to process message.");
                handlerInfo.handler.onMessage(byteBuffer, new Reply(this.flutterJNI, i8));
                return;
            } catch (Error e8) {
                handleError(e8);
                return;
            } catch (Exception e9) {
                Log.e(TAG, "Uncaught exception in binary message listener", e9);
            }
        } else {
            Log.v(TAG, "No registered handler for message. Responding to Dart with empty reply message.");
        }
        this.flutterJNI.invokePlatformMessageEmptyResponseCallback(i8);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public /* synthetic */ void lambda$dispatchMessageToQueue$0(String str, int i8, HandlerInfo handlerInfo, ByteBuffer byteBuffer, long j8) {
        TraceSection.endAsyncSection("PlatformChannel ScheduleHandler on " + str, i8);
        try {
            TraceSection traceSectionScoped = TraceSection.scoped("DartMessenger#handleMessageFromDart on " + str);
            try {
                invokeHandler(handlerInfo, byteBuffer, i8);
            ...
            this.flutterJNI.cleanupMessageData(j8);
        }
    }
    ...
    public void handleMessageFromDart(String str, ByteBuffer byteBuffer, int i8, long j8) {
        ...
        synchronized (this.handlersLock) {
            try {
                ...
                z7 = this.enableBufferingIncomingMessages.get() && handlerInfo == null;
                if (z7) {
                    if (!this.bufferedMessages.containsKey(str)) {
                        this.bufferedMessages.put(str, new LinkedList());
                    }
                    this.bufferedMessages.get(str).add(new BufferedMessageInfo(byteBuffer, i8, j8));
                }
            } catch (Throwable th) {
                throw th;
            }
        ...
        dispatchMessageToQueue(str, handlerInfo, byteBuffer, i8, j8);
    }

    @Override // io.flutter.embedding.engine.dart.PlatformMessageHandler
    public void handlePlatformMessageResponse(int i8, ByteBuffer byteBuffer) {
        Log.v(TAG, "Received message reply from Dart.");
        BinaryMessenger.BinaryReply binaryReplyRemove = this.pendingReplies.remove(Integer.valueOf(i8));
        if (binaryReplyRemove != null) {
            try {
                ...
                Log.e(TAG, "Uncaught exception in binary message reply handler", e9);
            }
        }
    }
    ...
    public void send(String str, ByteBuffer byteBuffer, BinaryMessenger.BinaryReply binaryReply) {
        ...
        try {
            ...
            int i8 = this.nextReplyId;
            this.nextReplyId = i8 + 1;
            if (binaryReply != null) {
                this.pendingReplies.put(Integer.valueOf(i8), binaryReply);
            }
            if (byteBuffer == null) {
                this.flutterJNI.dispatchEmptyPlatformMessage(str, i8);
            } else {
                this.flutterJNI.dispatchPlatformMessage(str, byteBuffer, byteBuffer.position(), i8);
            }
            if (traceSectionScoped != null) {
                traceSectionScoped.close();
            }
