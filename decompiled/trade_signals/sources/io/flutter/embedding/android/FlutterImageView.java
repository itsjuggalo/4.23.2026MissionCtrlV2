...
public class FlutterImageView extends View implements RenderSurface {
    ...
    private static ImageReader createImageReader(int i8, int i9) {
        int i10;
        int i11;
        if (i8 <= 0) {
            logW("ImageReader width must be greater than 0, but given width=%d, set width=1", Integer.valueOf(i8));
            i10 = 1;
        } else {
            i10 = i8;
        }
        if (i9 <= 0) {
            logW("ImageReader height must be greater than 0, but given height=%d, set height=1", Integer.valueOf(i9));
            i11 = 1;
        } else {
            i11 = i9;
        }
    ...
    public void onSizeChanged(int i8, int i9, int i10, int i11) {
        if (!(i8 == this.imageReader.getWidth() && i9 == this.imageReader.getHeight()) && this.kind == SurfaceKind.background && this.isAttachedToFlutterRenderer) {
    ...
    public void resizeIfNeeded(int i8, int i9) {
        ...
        if (i8 == this.imageReader.getWidth() && i9 == this.imageReader.getHeight()) {
            return;
        }
        closeCurrentImage();
        closeImageReader();
        this.imageReader = createImageReader(i8, i9);
    }
    ...
    public FlutterImageView(Context context, int i8, int i9, SurfaceKind surfaceKind) {
        this(context, createImageReader(i8, i9), surfaceKind);
    }
