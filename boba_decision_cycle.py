...
def build_boba_prompt(account, positions, shortlist_with_kronos, remaining_budget=3):
    ...
    try:
        ...
        if _web_ctx:
            ...
            print(f"[web-search] injected {_web_ctx.count('###')} ticker blocks into Boba prompt", flush=True)
...
def call_boba(prompt):
    ...
    # claude_oauth = direct Anthropic API on the sub token (fast, no Node cold-start);
    # claude_cli = slow CLI fallback (still sub); deepseek = cheap secondary backup.
    r = call_llm(full_prompt, ["claude_oauth", "claude_cli", "grok_oauth"])
