...
const FUNNEL = [
  ...
  { label: 'Trades', value: 3, color: '#6c5ce7' },
  ...
  return (
    <div style={{ padding: '24px 28px', fontFamily: "'Inter', -apple-system, sans-serif", background: '#0a0a10', minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#e8e8ed', margin: 0 }}>Performance</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        {MOCK_METRICS.map((agent) => (
          <div key={agent.name} style={{ background: '#0f0f17', border: `1px solid ${agent.color}30`, borderRadius: 12, padding: '18px 20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {agent.metrics.map((m) => (
                <div key={m.label} style={{ background: '#13131e', borderRadius: 8, padding: '10px 12px' }}>
                  <div style={{ fontSize: 10, color: '#5c5c72', fontFamily: "'JetBrains Mono', monospace", marginBottom: 4 }}>{m.label}</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: '#e8e8ed', fontFamily: "'JetBrains Mono', monospace" }}>{m.value}</div>
                  {m.sub && <div style={{ fontSize: 10, color: '#8b8b9e', marginTop: 2 }}>{m.sub}</div>}
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ background: '#0f0f17', border: '1px solid #1e1e2a', borderRadius: 12, padding: '18px 20px' }}>
        <div style={{ fontSize: 12, color: '#8b8b9e', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.5px', marginBottom: 16 }}>TEAM PIPELINE FUNNEL</div>
          ...
          {FUNNEL.map((f, i) => {
            ...
            return (
              <div key={f.label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ fontSize: 11, color: '#8b8b9e', fontFamily: "'JetBrains Mono', monospace" }}>{f.label}</span>
                {i < FUNNEL.length - 1 && (
                  <span style={{ fontSize: 10, color: '#5c5c72', position: 'absolute', marginLeft: 80 }}>→</span>
                )}
              </div>
            );
          })}
        </div>
        <div style={{ textAlign: 'center', marginTop: 12, fontSize: 11, color: '#5c5c72', fontFamily: "'JetBrains Mono', monospace" }}>
