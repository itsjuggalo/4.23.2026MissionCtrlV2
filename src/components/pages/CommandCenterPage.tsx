...
export function CommandCenterPage() {
  ...
  const [_pipelineFeed, setPipelineFeed] = useState<Record<string, unknown>[]>([]);
    ...
    <div style={{ padding: '20px', maxWidth: '1400px', margin: '0 auto', fontFamily: 'var(--font-mc-sans)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', maxHeight: '380px', overflowY: 'auto' }}>
            {(() => {
              ...
              return [
                ...
                const expStr = a.Expiry ? new Date(a.Expiry * 1000).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric' }) : '';
        ...
        <div className="cc" style={{ padding: '16px', maxHeight: '350px', overflowY: 'auto' }}>
          ...
          {(() => {
            if (unusualFlows.length === 0) {
              return <div style={{ color: '#455a64', fontSize: 'var(--mc-font-badge)', fontFamily: 'var(--font-mc-mono)', textAlign: 'center', padding: '30px' }}>No unusual flow detected</div>;
            }
            const cat = (f: any): string => {
              const v = f.Value || 0;
              if (v >= 1_000_000) return 'huge';
              if (v >= 500_000) return 'unusual';
              return 'small';
            };
            const catColor = (c: string) => c === 'huge' ? '#e040fb' : c === 'unusual' ? '#ffd600' : c === 'small' ? '#4fc3f7' : '#607d8b';
            return [
              <div key="legend" style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', padding: '4px 8px 8px', borderBottom: '1px solid #1a3a4a', marginBottom: '4px', fontSize: '10px', fontFamily: 'var(--font-mc-mono)', color: '#607d8b' }}>
                <span><span style={{ color: '#66bb6a', fontWeight: 700 }}>▲</span> bullish</span>
                <span><span style={{ color: '#ef5350', fontWeight: 700 }}>▼</span> bearish</span>
              ...
              ...unusualFlows.map((f: any, i: number) => {
                ...
                const isBlock = f.BlockType === 'BLOCK' || f.BlockType === 'BLOCKS';
                const c = cat(f);
                const fmtV = (n: number) => n >= 1000000 ? '$' + (n/1000000).toFixed(2) + 'M' : '$' + (n/1000).toFixed(0) + 'K';
                const expStr = f.ExpiryStr ? (() => { const d = new Date(f.ExpiryStr + 'T00:00:00'); return (d.getMonth()+1) + '/' + d.getDate(); })() : '';
                const ago = (() => { const m = Math.floor((Date.now() - (f.Time||0)) / 60000); return m < 1 ? 'now' : m < 60 ? m+'m' : m < 1440 ? Math.floor(m/60)+'h' : Math.floor(m/1440)+'d'; })();
                return (
                  <div key={i} onClick={() => setSelectedItem({ type: 'huge', data: { ...f, totalFlowValue: f.Value, SWEEPS: isSweep ? 1 : 0, BLOCKS: isBlock ? 1 : 0, AlertType: f.BlockType || 'unusual_flow', AlertPrice: f.Spot || null, isBullish: bullish, DTE: f.ExpiryStr ? Math.max(0, Math.ceil((new Date(f.ExpiryStr + 'T00:00:00').getTime() - Date.now()) / 86400000)) : null } })} style={{ cursor: 'pointer', display: 'grid', gridTemplateColumns: '34px 42px 50px 82px 44px 1fr 90px', gap: '6px', alignItems: 'center', padding: '5px 8px', borderLeft: '2px solid ' + catColor(c), background: '#0d1117', fontSize: 'var(--mc-font-label)', fontFamily: 'var(--font-mc-mono)' }}>
                    <span style={{ color: '#607d8b' }}>{ago}</span>
                    <span style={{ color: catColor(c), fontWeight: 700, fontSize: '9px', textTransform: 'uppercase' }}>{c}</span>
                    <span style={{ color: '#e0e0e0', fontWeight: 700 }}>{f.Symbol}</span>
                    <span style={{ color: isCall ? '#66bb6a' : '#ef5350', fontWeight: 600, whiteSpace: 'nowrap' }} title={isCall ? 'CALL option' : 'PUT option'}>{isCall ? 'C' : 'P'} ${f.Strike}</span>
                    <span style={{ color: '#90a4ae' }}>{expStr}</span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '8px', overflow: 'hidden', whiteSpace: 'nowrap' }}>
                      <span style={{ color: bullish ? '#66bb6a' : '#ef5350', fontWeight: 700 }} title={bullish ? 'Bullish flow (buyers in control)' : 'Bearish flow (sellers in control)'}>{bullish ? '▲' : '▼'}</span>
                      {isSweep && <span title="Sweep - aggressive multi-exchange fills (urgency)" style={{ color: '#ff9800', fontWeight: 600 }}>⚡1</span>}
                      {isBlock && <span title="Block - large single negotiated trade (institutional)" style={{ color: '#ce93d8', fontWeight: 600 }}>▦1</span>}
                    </span>
                    <span style={{ color: catColor(c), fontWeight: 800, fontSize: 'var(--mc-font-md)', textAlign: 'right' }}>{fmtV(f.Value)}</span>
                  </div>
                );
              })
            ];
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
        <div className="cc" style={{ padding: '16px', maxHeight: '350px', overflowY: 'auto' }}>
          ...
          {(() => {
            if (hugeAlerts.length === 0) {
              return <div style={{ color: '#455a64', fontSize: 'var(--mc-font-badge)', fontFamily: 'var(--font-mc-mono)', textAlign: 'center', padding: '30px' }}>No huge-flow alerts today</div>;
            }
            const cat = (a: any): string => {
              const t = (a.AlertType || '').toLowerCase();
              if (t.includes('etf')) return 'etf';
              if (t.includes('weekly')) return 'weekly';
              if (t.includes('repeat')) return 'repeat';
            ...
            const catColor = (c: string) => c === 'huge' ? '#e040fb' : c === 'unusual' ? '#ffd600' : c === 'repeat' ? '#4fc3f7' : c === 'weekly' ? '#ff9800' : c === 'etf' ? '#66bb6a' : '#607d8b';
            return [
              <div key="legend" style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', padding: '4px 8px 8px', borderBottom: '1px solid #1a3a4a', marginBottom: '4px', fontSize: '10px', fontFamily: 'var(--font-mc-mono)', color: '#607d8b' }}>
                <span><span style={{ color: '#66bb6a', fontWeight: 700 }}>▲</span> bullish</span>
                <span><span style={{ color: '#ef5350', fontWeight: 700 }}>▼</span> bearish</span>
              ...
              ...hugeAlerts.slice(0, 30).map((a: any, i: number) => {
                const v = a.totalFlowValue || 0;
                const c = cat(a);
                const fmtV = (n: number) => n >= 1000000 ? '$' + (n/1000000).toFixed(2) + 'M' : '$' + (n/1000).toFixed(0) + 'K';
                const expStr = a.Expiry ? new Date(a.Expiry * 1000).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric' }) : '';
                const ago = (() => { const ts = a.Updated || a.Time || 0; if (!ts) return ''; const m = Math.floor((Date.now() - ts) / 60000); return m < 1 ? 'now' : m < 60 ? m+'m' : m < 1440 ? Math.floor(m/60)+'h' : Math.floor(m/1440)+'d'; })();
                return (
                  <div key={i} onClick={() => setSelectedItem({ type: 'huge', data: a })} style={{ cursor: 'pointer', display: 'grid', gridTemplateColumns: '34px 42px 50px 82px 44px 1fr 90px', gap: '6px', alignItems: 'center', padding: '5px 8px', borderLeft: '2px solid ' + catColor(c), background: '#0d1117', fontSize: 'var(--mc-font-label)', fontFamily: 'var(--font-mc-mono)' }}>
                    <span style={{ color: '#607d8b' }}>{ago}</span>
                    <span style={{ color: catColor(c), fontWeight: 700, fontSize: '9px', textTransform: 'uppercase' }}>{c}</span>
                    <span style={{ color: '#e0e0e0', fontWeight: 700 }}>{a.Symbol}</span>
                    <span style={{ color: a.OptionType === 'CALL' ? '#66bb6a' : '#ef5350', fontWeight: 600, whiteSpace: 'nowrap' }} title={a.OptionType === 'CALL' ? 'CALL option' : 'PUT option'}>{a.OptionType === 'CALL' ? 'C' : 'P'} ${a.Strike}</span>
      ...
      {selectedItem && (
        <div style={{
          ...
          {(selectedItem.type === 'alert' || selectedItem.type === 'huge') && (() => {
            ...
            const v = a.totalFlowValue || 0;
            const fmtV = (n: number) => n >= 1000000 ? '$' + (n/1000000).toFixed(2) + 'M' : '$' + (n/1000).toFixed(0) + 'K';
            const expStr = a.Expiry ? new Date(a.Expiry * 1000).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—';
            return (
