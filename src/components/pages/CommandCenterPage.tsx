...
const STATUS_COLOR = { online: '#2ed573', idle: '#ffa502', offline: '#ff4757' };
const TYPE_COLOR = { alert: '#ff4757', signal: '#6c5ce7', trade: '#2ed573', info: '#636e72' };
const SOURCE_COLOR: Record<string, string> = { BOBA: '#6c5ce7', JAZZYHAZZY: '#0984e3', ORION: '#e17055' };
  ...
  return (
    <div style={{ padding: '24px 28px', fontFamily: "'Inter', -apple-system, sans-serif", minHeight: '100vh', background: '#0a0a10' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#e8e8ed', margin: 0, letterSpacing: '-0.3px' }}>Command Center</h1>
      ...
      {/* Mission Progress */}
      <div style={{ background: '#0f0f17', border: '1px solid #1e1e2a', borderRadius: 12, padding: '18px 20px', marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <span style={{ fontSize: 12, color: '#8b8b9e', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.5px' }}>MISSION PROGRESS</span>
          <span style={{ fontSize: 12, color: '#6c5ce7', fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 }}>Phase 1 of 2</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
          <span style={{ fontSize: 20, fontWeight: 700, color: '#e8e8ed', fontFamily: "'JetBrains Mono', monospace" }}>${portfolioValue.toLocaleString()}</span>
          <span style={{ fontSize: 14, color: '#5c5c72', fontFamily: "'JetBrains Mono', monospace" }}>→ ${targetValue.toLocaleString()} goal</span>
        </div>
        <div style={{ background: '#1e1e2a', borderRadius: 6, height: 8, overflow: 'hidden' }}>
          <div style={{ background: 'linear-gradient(90deg, #6c5ce7, #a29bfe)', width: `${Math.max(1, progress)}%`, height: '100%', borderRadius: 6, transition: 'width 0.5s ease' }} />
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        {agents.map((agent) => (
          <div key={agent.name} style={{ background: '#0f0f17', border: `1px solid ${STATUS_COLOR[agent.status]}30`, borderRadius: 12, padding: '16px 18px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#e8e8ed' }}>{agent.name}</div>
                <div style={{ fontSize: 11, color: '#8b8b9e', marginTop: 2 }}>{agent.role}</div>
              </div>
            ...
            <div style={{ fontSize: 11, color: '#5c5c72', fontFamily: "'JetBrains Mono', monospace", marginBottom: 8, lineHeight: 1.5 }}>
              {agent.lastAction}
            </div>
            <div style={{ fontSize: 11, color: '#8b8b9e' }}>
              <span style={{ color: '#6c5ce7', fontWeight: 700 }}>{agent.tasksToday}</span> tasks today
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: '#0f0f17', border: '1px solid #1e1e2a', borderRadius: 12, padding: '18px 20px' }}>
        <div style={{ fontSize: 12, color: '#8b8b9e', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.5px', marginBottom: 14 }}>LIVE PIPELINE</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {pipeline.map((event, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '8px 12px', background: '#13131e', borderRadius: 8, borderLeft: `3px solid ${TYPE_COLOR[event.type]}` }}>
              <span style={{ fontSize: 10, color: '#5c5c72', fontFamily: "'JetBrains Mono', monospace", flexShrink: 0, marginTop: 2 }}>{event.time}</span>
              <span style={{ fontSize: 10, color: SOURCE_COLOR[event.source] || '#8b8b9e', fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, flexShrink: 0 }}>[{event.source}]</span>
