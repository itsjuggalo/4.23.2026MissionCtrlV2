...
export function PowerTraderPage() {
  ...
  const pnlColor = (v: number | null | undefined) => (v ?? 0) >= 0 ? '#00d2a0' : '#ff4757';
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', color: '#5c5c72', fontFamily: 'monospace' }}>
      Loading PowerTrader...
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h1 style={{ color: '#e8e8ed', fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: 24, margin: 0 }}>
            PowerTrader AI
          </h1>
          <p style={{ color: '#5c5c72', fontFamily: 'monospace', fontSize: 12, margin: '4px 0 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{
            ...
            background: running ? '#00d2a022' : '#ff475722',
            border: `1px solid ${running ? '#00d2a044' : '#ff475744'}`,
          }}>
            <div style={{
              width: 8, height: 8, borderRadius: '50%',
              background: running ? '#00d2a0' : '#ff4757',
              boxShadow: `0 0 6px ${running ? '#00d2a0' : '#ff4757'}`,
            }} />
            <span style={{ color: running ? '#00d2a0' : '#ff4757', fontFamily: 'monospace', fontSize: 12, fontWeight: 600 }}>
          ...
          <div style={{
            padding: '8px 16px', borderRadius: 8,
            background: '#6c5ce722', border: '1px solid #6c5ce744',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {[
          { label: 'Account Value',    value: fmt.currency(status.account_value),  color: '#e8e8ed' },
          { label: 'Buying Power',     value: fmt.currency(status.buying_power),   color: '#3b82f6' },
          { label: 'Total P&L',        value: fmt.currency(status.total_pnl),      color: pnlColor(status.total_pnl) },
          { label: 'Open Positions',   value: String(status.open_positions ?? 0),  color: '#f59e0b' },
        ].map(card => (
          <div key={card.label} style={{
            background: '#111118', border: '1px solid #1e1e2a',
            borderRadius: 10, padding: '14px 16px',
          }}>
            <div style={{ fontSize: 11, color: '#5c5c72', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              {card.label}
            </div>
          ...
          return (
            <div key={coin} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <span style={{ color: '#e8e8ed', fontFamily: 'monospace', fontWeight: 700, fontSize: 14 }}>{coin}</span>
                {holding && (
              ...
              {coinPnl && (
                <div style={{ marginTop: 6, paddingTop: 6, borderTop: '1px solid #1e1e2a', fontSize: 10, color: '#5c5c72', fontFamily: 'monospace' }}>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid #1e1e2a', paddingBottom: 0 }}>
      ...
      {tab === 'overview' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {/* P&L summary */}
          <div style={{ background: '#111118', border: '1px solid #1e1e2a', borderRadius: 10, padding: '16px' }}>
            <div style={{ color: '#5c5c72', fontFamily: 'monospace', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 12 }}>
              P&L Summary
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div>
                <div style={{ color: '#5c5c72', fontSize: 11, fontFamily: 'monospace' }}>Realized</div>
              ...
              <div>
                <div style={{ color: '#5c5c72', fontSize: 11, fontFamily: 'monospace' }}>Unrealized</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {trades.slice(0, 5).map((trade, i) => (
                  <div key={i} style={{
                    ...
                    padding: '8px 10px', background: '#0d0d14', borderRadius: 6,
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{
                        padding: '2px 8px', borderRadius: 4, fontSize: 10, fontWeight: 700, fontFamily: 'monospace',
                        background: trade.side === 'buy' ? '#00d2a022' : '#ff475722',
                        color: trade.side === 'buy' ? '#00d2a0' : '#ff4757',
                      }}>
                        {(trade.side || '').toUpperCase()}
                      </span>
                      <span style={{ color: '#e8e8ed', fontFamily: 'monospace', fontSize: 12, fontWeight: 600 }}>
                        {trade.coin || trade.symbol}
                      </span>
                      <span style={{ color: '#5c5c72', fontFamily: 'monospace', fontSize: 11 }}>
          ...
          {!running && (
            <div style={{
              background: '#1a1a0a', border: '1px solid #f59e0b44',
              borderRadius: 10, padding: '16px',
            }}>
              <div style={{ color: '#f59e0b', fontFamily: 'monospace', fontSize: 12, fontWeight: 600, marginBottom: 8 }}>
                ⚠️ PowerTrader not running
              </div>
              <div style={{ color: '#8b8b9e', fontFamily: 'monospace', fontSize: 11, lineHeight: 1.8 }}>
          ...
          {trades.length === 0 ? (
            ...
            <div style={{ maxHeight: 500, overflowY: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'monospace', fontSize: 12 }}>
                <thead>
                  <tr style={{ color: '#5c5c72', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    {['Time','Side','Coin','Qty','Price','Cost','P&L','DCA Level'].map(h => (
                      <th key={h} style={{ textAlign: 'left', padding: '0 12px 8px 0', borderBottom: '1px solid #1e1e2a' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {trades.map((trade, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #1e1e2a22' }}>
                      <td style={{ padding: '8px 12px 8px 0', color: '#3c3c52' }}>{fmt.time(trade.ts || trade.timestamp)}</td>
                      <td style={{ padding: '8px 12px 8px 0' }}>
                        <span style={{ color: trade.side === 'buy' ? '#00d2a0' : '#ff4757', fontWeight: 600 }}>
                          {(trade.side || '').toUpperCase()}
                        </span>
                      </td>
                      <td style={{ padding: '8px 12px 8px 0', color: '#e8e8ed', fontWeight: 600 }}>{trade.coin || trade.symbol}</td>
                      <td style={{ padding: '8px 12px 8px 0', color: '#8b8b9e' }}>{fmt.qty(trade.qty)}</td>
                      <td style={{ padding: '8px 12px 8px 0', color: '#8b8b9e' }}>{fmt.currency(trade.price)}</td>
                      <td style={{ padding: '8px 12px 8px 0', color: '#8b8b9e' }}>{fmt.currency(trade.cost)}</td>
                      <td style={{ padding: '8px 12px 8px 0', color: pnlColor(trade.pnl) }}>{trade.pnl != null ? fmt.currency(trade.pnl) : '—'}</td>
                      <td style={{ padding: '8px 12px 8px 0', color: '#5c5c72' }}>{trade.dca_level ?? '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'monospace', fontSize: 12 }}>
              ...
              <tbody>
                {Object.entries(status.holdings).map(([coin, h]) => (
                  <tr key={coin} style={{ borderBottom: '1px solid #1e1e2a22' }}>
                    <td style={{ padding: '8px 12px 8px 0', color: '#e8e8ed', fontWeight: 700 }}>{coin}</td>
                    <td style={{ padding: '8px 12px 8px 0', color: '#8b8b9e' }}>{fmt.qty(h.qty)}</td>
                    <td style={{ padding: '8px 12px 8px 0', color: '#8b8b9e' }}>{fmt.currency(h.avg_cost)}</td>
                    <td style={{ padding: '8px 12px 8px 0', color: '#8b8b9e' }}>{fmt.currency(h.current_price)}</td>
                    <td style={{ padding: '8px 12px 8px 0', color: '#e8e8ed' }}>{fmt.currency(h.qty * h.current_price)}</td>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ background: '#111118', border: '1px solid #1e1e2a', borderRadius: 10, padding: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontFamily: 'monospace', fontSize: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #1e1e2a' }}>
                <span style={{ color: '#5c5c72' }}>Trader Script</span>
                <span style={{ color: '#a78bfa' }}>powertrader-ai/pt_trader.py</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #1e1e2a' }}>
                <span style={{ color: '#5c5c72' }}>Thinker Script</span>
                <span style={{ color: '#a78bfa' }}>powertrader-ai/pt_thinker.py</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #1e1e2a' }}>
                <span style={{ color: '#5c5c72' }}>Hub Data Dir</span>
                <span style={{ color: '#a78bfa' }}>powertrader-ai/hub_data/</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #1e1e2a' }}>
                <span style={{ color: '#5c5c72' }}>Robinhood Keys</span>
                <span style={{ color: running ? '#00d2a0' : '#ff4757' }}>{running ? '✅ Configured' : '❌ Not configured'}</span>
              </div>
            </div>
          </div>

          <div style={{ background: '#0d1117', border: '1px solid #1e1e2a', borderRadius: 10, padding: '16px' }}>
            <div style={{ color: '#5c5c72', fontFamily: 'monospace', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                ...
                <div key={item.label} style={{ padding: '8px 12px', background: '#111118', borderRadius: 6 }}>
                  <div style={{ color: '#5c5c72', fontSize: 10, marginBottom: 4 }}>{item.label}</div>
