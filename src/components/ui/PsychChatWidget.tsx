  ...
  return (
    <>
      <style>{`
        ...
        .psych-scroll::-webkit-scrollbar { width: 3px; } .psych-scroll::-webkit-scrollbar-track { background: transparent; } .psych-scroll::-webkit-scrollbar-thumb { background: #1e1e2a; border-radius: 3px; }
      `}</style>

      {isOpen && (
        <div style={{ position: 'fixed', bottom: 88, right: 24, width: 370, height: 520, background: '#0f0f17', borderRadius: 12, border: '1px solid #1e1e2a', boxShadow: '0 20px 60px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'psychSlideUp 0.25s ease-out', zIndex: 9999 }}>
          {/* Header */}
          <div style={{ padding: '14px 16px', background: '#111118', borderBottom: '1px solid #1e1e2a', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            <div style={{ position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', bottom: -1, right: -1, width: 9, height: 9, borderRadius: '50%', background: '#00d2a0', border: '2px solid #111118', boxShadow: '0 0 4px #00d2a0' }} />
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#e8e8ed' }}>PsychTechologist</div>
            ...
            <button onClick={() => setIsOpen(false)} style={{ background: 'rgba(232,232,237,0.03)', border: '1px solid #1e1e2a', borderRadius: 6, color: '#5c5c72', cursor: 'pointer', width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>−</button>
          </div>
          ...
          <div className="psych-scroll" style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 6 }}>
            {messages.map((msg, i) => {
              if (msg.role === 'alert') return (
                <div key={i} style={{ padding: '10px 12px', background: 'rgba(139,92,246,0.05)', border: '1px solid rgba(139,92,246,0.1)', borderRadius: 8, fontSize: 12, lineHeight: 1.5, color: '#8b8b9e' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                    ...
                    <span style={{ fontSize: 10, fontFamily: 'var(--font-jetbrains-mono), monospace', color: '#5c5c72', marginLeft: 'auto' }}>{msg.time}</span>
                  </div>
                  {msg.message}
                </div>
              );
              ...
              return (
                <div key={i} style={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start' }}>
                  <div style={{ maxWidth: '84%', padding: '9px 13px', borderRadius: isUser ? '12px 12px 3px 12px' : '12px 12px 12px 3px', background: isUser ? '#8B5CF6' : '#111118', border: isUser ? 'none' : '1px solid #1e1e2a', color: isUser ? '#fff' : '#e8e8ed', fontSize: 13, lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{msg.content}</div>
                </div>
              );
            })}
            {isTyping && (
              <div style={{ display: 'flex' }}>
                <div style={{ padding: '10px 16px', borderRadius: '12px 12px 12px 3px', background: '#111118', border: '1px solid #1e1e2a', display: 'flex', gap: 4 }}>
          ...
          <div style={{ padding: '10px 14px 14px', display: 'flex', gap: 8, alignItems: 'flex-end', flexShrink: 0 }}>
            <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Talk to PsychTechologist..." rows={1} style={{ flex: 1, background: '#111118', border: '1px solid #1e1e2a', borderRadius: 8, padding: '9px 12px', color: '#e8e8ed', fontSize: 13, resize: 'none', lineHeight: 1.4, maxHeight: 72, outline: 'none' }} />
            <button onClick={() => sendMessage()} disabled={!input.trim() || isTyping} style={{ width: 36, height: 36, borderRadius: 8, background: input.trim() ? '#8B5CF6' : 'rgba(232,232,237,0.03)', border: input.trim() ? 'none' : '1px solid #1e1e2a', cursor: input.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M22 2L11 13M22 2L15 22L11 13M22 2L2 9L11 13" stroke={input.trim() ? '#fff' : '#5c5c72'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
            </button>
          </div>
        </div>
      )}
      ...
      <button onClick={() => setIsOpen(!isOpen)} style={{ position: 'fixed', bottom: 24, right: 24, width: 54, height: 54, borderRadius: 14, background: 'linear-gradient(135deg, #8B5CF6, #6D28D9)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(139,92,246,0.3)', animation: unreadCount > 0 && !isOpen ? 'psychPulse 2s ease-in-out infinite' : 'none', zIndex: 10000 }}>
        ...
        {unreadCount > 0 && !isOpen && (
          <div style={{ position: 'absolute', top: -3, right: -3, width: 18, height: 18, borderRadius: '50%', background: '#ff4757', color: '#fff', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #0a0a0f', fontFamily: 'var(--font-jetbrains-mono), monospace' }}>{unreadCount}</div>
        )}
      </button>
    </>
  );
