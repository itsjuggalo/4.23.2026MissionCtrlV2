package com.google.protobuf;

import com.google.protobuf.C;
import java.util.List;
import java.util.Map;

/* JADX INFO: loaded from: classes.dex */
public interface m0 {

    public enum a {
        ASCENDING,
        DESCENDING
    }

    void A(int i8, float f8);

    void B(int i8);

    void C(int i8, List list, boolean z7);

    void D(int i8, int i9);

    void E(int i8, List list, boolean z7);

    void F(int i8, List list, boolean z7);

    void G(int i8, List list, boolean z7);

    void H(int i8, int i9);

    void I(int i8, List list);

    void J(int i8, List list, Y y7);

    void K(int i8, List list, Y y7);

    void L(int i8, Object obj, Y y7);

    void M(int i8, Object obj, Y y7);

    void N(int i8, AbstractC1467f abstractC1467f);

    void O(int i8, C.a aVar, Map map);

    void a(int i8, List list, boolean z7);

    void b(int i8, int i9);

    void c(int i8, Object obj);

    void d(int i8, int i9);

    void e(int i8, double d8);

    void f(int i8, List list, boolean z7);

    void g(int i8, List list, boolean z7);

    void h(int i8, long j8);

    a i();

    void j(int i8, List list);

    void k(int i8, String str);

    void l(int i8, long j8);

    void m(int i8, List list, boolean z7);

    void n(int i8, long j8);

    void o(int i8, boolean z7);

    void p(int i8, int i9);

    void q(int i8);

    void r(int i8, int i9);

    void s(int i8, List list, boolean z7);

    void t(int i8, List list, boolean z7);

    void u(int i8, long j8);

    void v(int i8, List list, boolean z7);

    void w(int i8, List list, boolean z7);

    void x(int i8, List list, boolean z7);

    void y(int i8, List list, boolean z7);

    void z(int i8, long j8);
}
