...
public final class zzaoo {
    ...
    public static zzans zza(String str) throws ParseException {
            ...
            if (iIndexOf2 != -1) {
                ...
                if (str.charAt(iIndexOf2) == 'Z') {
                    if (str.length() != iIndexOf2 + 1) {
                        throw new ParseException(AbstractC1024h.c("Failed to parse timestamp: invalid trailing data \"", str.substring(iIndexOf2), "\""), 0);
                    }
                } else {
                ...
                try {
                    if (zza(time)) {
                        if (iCharAt <= -1000000000 || iCharAt >= 1000000000) {
                            long j4 = iCharAt / 1000000000;
                            long j6 = time + j4;
                            if (((j4 ^ time) < 0) | ((time ^ j6) >= 0)) {
                                iCharAt %= 1000000000;
                                time = j6;
                            } else {
                    ...
                    ParseException parseException = new ParseException(AbstractC1024h.c("Failed to parse timestamp ", str, " Timestamp is out of range."), 0);
                    parseException.initCause(e);
                    throw parseException;
                }
            }
        ...
        throw new ParseException(AbstractC1024h.c("Failed to parse timestamp: invalid timestamp \"", str, "\""), 0);
    }
