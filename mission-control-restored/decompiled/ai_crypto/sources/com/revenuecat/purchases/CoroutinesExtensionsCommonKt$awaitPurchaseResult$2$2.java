package com.revenuecat.purchases;

import E5.E;
import E5.p;
import E5.q;
import Q5.o;
import kotlin.jvm.internal.r;
import kotlin.jvm.internal.s;

/* JADX INFO: loaded from: classes.dex */
public final class CoroutinesExtensionsCommonKt$awaitPurchaseResult$2$2 extends s implements o {
    final /* synthetic */ H5.d $continuation;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public CoroutinesExtensionsCommonKt$awaitPurchaseResult$2$2(H5.d dVar) {
        super(2);
        this.$continuation = dVar;
    }

    @Override // Q5.o
    public /* bridge */ /* synthetic */ Object invoke(Object obj, Object obj2) {
        invoke((PurchasesError) obj, ((Boolean) obj2).booleanValue());
        return E.f1657a;
    }

    public final void invoke(PurchasesError purchasesError, boolean z7) {
        r.f(purchasesError, "purchasesError");
        H5.d dVar = this.$continuation;
        p.a aVar = p.f1681b;
        dVar.resumeWith(p.b(p.a(p.b(q.a(new PurchasesTransactionException(purchasesError, z7))))));
    }
}
