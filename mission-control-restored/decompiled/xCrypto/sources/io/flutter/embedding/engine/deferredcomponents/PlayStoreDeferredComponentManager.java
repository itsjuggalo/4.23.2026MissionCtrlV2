...
public class PlayStoreDeferredComponentManager implements DeferredComponentManager {
    ...
    private void initLoadingUnitMappingToComponentNames() {
        ...
        for (String str2 : string.split(f.f8804a)) {
            String[] strArrSplit = str2.split(Constants.SUBS_ID_BASE_PLAN_ID_SEPARATOR, -1);
            int i4 = Integer.parseInt(strArrSplit[0]);
            this.loadingUnitIdToComponentNames.put(i4, strArrSplit[1]);
            if (strArrSplit.length > 2) {
                this.loadingUnitIdToSharedLibraryNames.put(i4, strArrSplit[2]);
            }
        }
    }

    private /* synthetic */ void lambda$installDeferredComponent$0(String str, int i4, Integer num) {
        this.sessionIdToName.put(num.intValue(), str);
        this.sessionIdToLoadingUnitId.put(num.intValue(), i4);
    ...
    private /* synthetic */ void lambda$installDeferredComponent$1(int i4, String str, Exception exc) {
        SplitInstallException splitInstallException = (SplitInstallException) exc;
        int errorCode = splitInstallException.getErrorCode();
        if (errorCode == -6) {
            this.flutterJNI.deferredComponentInstallFailure(i4, "Install of deferred component module \"" + str + "\" failed with a network error", true);
            return;
        }
        if (errorCode != -2) {
            this.flutterJNI.deferredComponentInstallFailure(i4, String.format("Install of deferred component module \"%s\" failed with error %d: %s", str, Integer.valueOf(splitInstallException.getErrorCode()), splitInstallException.getMessage()), false);
            return;
        }
        this.flutterJNI.deferredComponentInstallFailure(i4, "Install of deferred component module \"" + str + "\" failed as it is unavailable", false);
    }
    ...
    @Override // io.flutter.embedding.engine.deferredcomponents.DeferredComponentManager
    public String getDeferredComponentInstallState(int i4, String str) {
        if (str == null) {
            str = this.loadingUnitIdToComponentNames.get(i4);
        }
        if (str == null) {
    ...
    @Override // io.flutter.embedding.engine.deferredcomponents.DeferredComponentManager
    public void installDeferredComponent(final int i4, final String str) {
        final String str2 = str != null ? str : this.loadingUnitIdToComponentNames.get(i4);
        if (str2 == null) {
            Log.e(TAG, "Deferred component name was null and could not be resolved from loading unit id.");
        } else if (!str2.equals("") || i4 <= 0) {
            ...
            loadDartLibrary(i4, str2);
        }
    }

    @Override // io.flutter.embedding.engine.deferredcomponents.DeferredComponentManager
    public void loadAssets(int i4, String str) {
        if (verifyJNI()) {
            try {
    ...
    @Override // io.flutter.embedding.engine.deferredcomponents.DeferredComponentManager
    public void loadDartLibrary(int i4, String str) {
        if (verifyJNI() && i4 >= 0) {
            String str2 = this.loadingUnitIdToSharedLibraryNames.get(i4);
            if (str2 == null) {
                str2 = this.flutterApplicationInfo.aotSharedLibraryName + "-" + i4 + ".part.so";
            }
            ...
            this.flutterJNI.loadDartDeferredLibrary(i4, (String[]) arrayList3.toArray(new String[arrayList3.size()]));
        }
    }
    ...
    @Override // io.flutter.embedding.engine.deferredcomponents.DeferredComponentManager
    public boolean uninstallDeferredComponent(int i4, String str) {
        if (str == null) {
            str = this.loadingUnitIdToComponentNames.get(i4);
        }
        if (str == null) {
