...
public final class BasicMessageChannel<T> {
            ...
            try {
                this.handler.onMessage((T) BasicMessageChannel.this.codec.decodeMessage(byteBuffer), new Reply<T>() { // from class: io.flutter.plugin.common.BasicMessageChannel.IncomingMessageHandler.1
                    ...
                    public void reply(T t4) {
                        binaryReply.reply(BasicMessageChannel.this.codec.encodeMessage(t4));
                    }
                });
    ...
    public interface MessageHandler<T> {
        void onMessage(T t4, Reply<T> reply);
    }
    ...
    public void resizeChannelBuffer(int i4) {
        resizeChannelBuffer(this.messenger, this.name, i4);
    }

    public void send(T t4) {
        send(t4, null);
    }
    ...
    public static void resizeChannelBuffer(BinaryMessenger binaryMessenger, String str, int i4) {
        binaryMessenger.send(CHANNEL_BUFFERS_CHANNEL, packetFromEncodedMessage(StandardMethodCodec.INSTANCE.encodeMethodCall(new MethodCall("resize", Arrays.asList(str, Integer.valueOf(i4))))));
    }

    public static void setWarnsOnChannelOverflow(BinaryMessenger binaryMessenger, String str, boolean z4) {
        binaryMessenger.send(CHANNEL_BUFFERS_CHANNEL, packetFromEncodedMessage(StandardMethodCodec.INSTANCE.encodeMethodCall(new MethodCall("overflow", Arrays.asList(str, Boolean.valueOf(!z4))))));
    }
    ...
    java.lang.UnsupportedOperationException: ArgType.getObject(), call class: class jadx.core.dex.instructions.args.ArgType$UnknownArg
    	...
    	at jadx.core.dex.visitors.typeinference.FixTypesVisitor.visit(FixTypesVisitor.java:94)
     */
    public void send(T t4, Reply<T> reply) {
        this.messenger.send(this.name, this.codec.encodeMessage(t4), reply != null ? new IncomingReplyHandler(reply) : null);
    }
}
