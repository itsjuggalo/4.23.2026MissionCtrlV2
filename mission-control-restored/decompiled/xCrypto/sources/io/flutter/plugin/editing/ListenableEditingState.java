...
class ListenableEditingState extends SpannableStringBuilder {
    ...
    public SpannableStringBuilder replace(int i4, int i5, CharSequence charSequence, int i6, int i7) {
        ...
        this.mBatchTextEditingDeltas.add(new TextEditingDelta(string, i4, i5, charSequence, getSelectionStart(), getSelectionEnd(), getComposingStart(), getComposingEnd()));
        if (this.mBatchEditNestDepth > 0) {
            return spannableStringBuilderReplace;
        }
        notifyListenersIfNeeded(z4, (getSelectionStart() == selectionStart && getSelectionEnd() == selectionEnd) ? false : true, (getComposingStart() == composingStart && getComposingEnd() == composingEnd) ? false : true);
        return spannableStringBuilderReplace;
    }
}
