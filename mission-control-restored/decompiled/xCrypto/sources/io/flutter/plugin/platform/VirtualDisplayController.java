...
class VirtualDisplayController {
    ...
    private VirtualDisplayController(Context context, AccessibilityEventsDelegate accessibilityEventsDelegate, VirtualDisplay virtualDisplay, PlatformView platformView, PlatformViewRenderTarget platformViewRenderTarget, View.OnFocusChangeListener onFocusChangeListener, int i4, Object obj) {
        ...
        SingleViewPresentation singleViewPresentation = new SingleViewPresentation(context, this.virtualDisplay.getDisplay(), platformView, accessibilityEventsDelegate, i4, onFocusChangeListener);
        this.presentation = singleViewPresentation;
        singleViewPresentation.show();
    }

    public static VirtualDisplayController create(Context context, AccessibilityEventsDelegate accessibilityEventsDelegate, PlatformView platformView, PlatformViewRenderTarget platformViewRenderTarget, int i4, int i5, int i6, Object obj, View.OnFocusChangeListener onFocusChangeListener) {
        if (i4 == 0 || i5 == 0) {
            return null;
        }
        ...
        VirtualDisplay virtualDisplayCreateVirtualDisplay = displayManager.createVirtualDisplay("flutter-vd#" + i6, i4, i5, displayMetrics.densityDpi, platformViewRenderTarget.getSurface(), 0, callback, null);
        if (virtualDisplayCreateVirtualDisplay == null) {
            return null;
        }
        return new VirtualDisplayController(context, accessibilityEventsDelegate, virtualDisplayCreateVirtualDisplay, platformView, platformViewRenderTarget, onFocusChangeListener, i6, obj);
    }

    private void resize31(View view, int i4, int i5, Runnable runnable) {
        this.renderTarget.resize(i4, i5);
        this.virtualDisplay.resize(i4, i5, this.densityDpi);
    ...
    public void resize(int i4, int i5, final Runnable runnable) {
        if (i4 == getRenderTargetWidth() && i5 == getRenderTargetHeight()) {
            getView().postDelayed(runnable, 0L);
            return;
        }
        if (Build.VERSION.SDK_INT >= 31) {
            resize31(getView(), i4, i5, runnable);
            return;
        }
        boolean zIsFocused = getView().isFocused();
        SingleViewPresentation.PresentationState presentationStateDetachState = this.presentation.detachState();
        ...
        this.renderTarget.resize(i4, i5);
        this.virtualDisplay = displayManager.createVirtualDisplay("flutter-vd#" + this.viewId, i4, i5, this.densityDpi, this.renderTarget.getSurface(), 0, callback, null);
