package com.revenuecat.purchases.virtualcurrencies;

import F3.j;
import G3.a;
import I3.c;
import I3.d;
import I3.e;
import I3.f;
import J3.C;
import J3.C0327b0;
import J3.H;
import J3.k0;
import J3.o0;
import com.amazon.a.a.o.b;
import kotlin.jvm.internal.r;

/* JADX INFO: loaded from: classes3.dex */
public final class VirtualCurrency$$serializer implements C {
    public static final VirtualCurrency$$serializer INSTANCE;
    private static final /* synthetic */ C0327b0 descriptor;

    static {
        VirtualCurrency$$serializer virtualCurrency$$serializer = new VirtualCurrency$$serializer();
        INSTANCE = virtualCurrency$$serializer;
        C0327b0 c0327b0 = new C0327b0("com.revenuecat.purchases.virtualcurrencies.VirtualCurrency", virtualCurrency$$serializer, 4);
        c0327b0.l("balance", false);
        c0327b0.l("name", false);
        c0327b0.l("code", false);
        c0327b0.l(b.f8755c, true);
        descriptor = c0327b0;
    }

    private VirtualCurrency$$serializer() {
    }

    @Override // J3.C
    public F3.b[] childSerializers() {
        o0 o0Var = o0.f952a;
        return new F3.b[]{H.f874a, o0Var, o0Var, a.p(o0Var)};
    }

    @Override // F3.a
    public VirtualCurrency deserialize(e decoder) {
        int iH;
        int i4;
        String str;
        String str2;
        Object objY;
        r.f(decoder, "decoder");
        H3.e descriptor2 = getDescriptor();
        c cVarD = decoder.d(descriptor2);
        if (cVarD.w()) {
            iH = cVarD.h(descriptor2, 0);
            String strV = cVarD.v(descriptor2, 1);
            String strV2 = cVarD.v(descriptor2, 2);
            objY = cVarD.y(descriptor2, 3, o0.f952a, null);
            str2 = strV2;
            str = strV;
            i4 = 15;
        } else {
            boolean z4 = true;
            iH = 0;
            String strV3 = null;
            String strV4 = null;
            Object objY2 = null;
            int i5 = 0;
            while (z4) {
                int i6 = cVarD.i(descriptor2);
                if (i6 == -1) {
                    z4 = false;
                } else if (i6 == 0) {
                    iH = cVarD.h(descriptor2, 0);
                    i5 |= 1;
                } else if (i6 == 1) {
                    strV3 = cVarD.v(descriptor2, 1);
                    i5 |= 2;
                } else if (i6 == 2) {
                    strV4 = cVarD.v(descriptor2, 2);
                    i5 |= 4;
                } else {
                    if (i6 != 3) {
                        throw new j(i6);
                    }
                    objY2 = cVarD.y(descriptor2, 3, o0.f952a, objY2);
                    i5 |= 8;
                }
            }
            i4 = i5;
            str = strV3;
            str2 = strV4;
            objY = objY2;
        }
        int i7 = iH;
        cVarD.c(descriptor2);
        return new VirtualCurrency(i4, i7, str, str2, (String) objY, (k0) null);
    }

    @Override // F3.b, F3.h, F3.a
    public H3.e getDescriptor() {
        return descriptor;
    }

    @Override // F3.h
    public void serialize(f encoder, VirtualCurrency value) {
        r.f(encoder, "encoder");
        r.f(value, "value");
        H3.e descriptor2 = getDescriptor();
        d dVarD = encoder.d(descriptor2);
        VirtualCurrency.write$Self(value, dVarD, descriptor2);
        dVarD.c(descriptor2);
    }

    @Override // J3.C
    public F3.b[] typeParametersSerializers() {
        return C.a.a(this);
    }
}
