...
public final class zzaei {
    ...
    */
    ...
    public static FirebaseException zza(Status status, AuthCredential authCredential, String str, String str2) {
        int iK = status.k();
        if (iK != 17012 && iK != 17007 && iK != 17025) {
            return zza(status);
        }
        return new FirebaseAuthUserCollisionException(zza(iK), zza(zzb(iK), status)).zza(authCredential).zza(str);
    }
    ...
    private static String zza(String str, Status status) {
        if (TextUtils.isEmpty(status.l())) {
            return str;
        }
        return String.format(str + " [ %s ]", status.l());
    }
