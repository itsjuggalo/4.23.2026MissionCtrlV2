package com.google.android.gms.internal.p002firebaseauthapi;

import com.google.android.gms.common.api.Status;
import com.google.firebase.auth.PhoneAuthCredential;

/* JADX INFO: loaded from: classes.dex */
public interface zzaed {
    void zza();

    void zza(Status status);

    void zza(Status status, PhoneAuthCredential phoneAuthCredential);

    void zza(zzaaj zzaajVar);

    void zza(zzaak zzaakVar);

    void zza(zzagn zzagnVar);

    void zza(zzahj zzahjVar);

    void zza(zzahk zzahkVar);

    void zza(zzahn zzahnVar);

    void zza(zzahn zzahnVar, zzahc zzahcVar);

    void zza(zzahx zzahxVar);

    void zza(zzahy zzahyVar);

    void zza(zzaij zzaijVar);

    void zza(PhoneAuthCredential phoneAuthCredential);

    void zza(String str);

    void zzb();

    void zzb(String str);

    void zzc();

    void zzc(String str);
}
