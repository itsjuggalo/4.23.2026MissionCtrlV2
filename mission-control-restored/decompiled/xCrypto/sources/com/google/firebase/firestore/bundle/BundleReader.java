...
public class BundleReader {
    ...
    private int indexOfOpenBracket() {
        this.buffer.mark();
        for (int i4 = 0; i4 < this.buffer.remaining(); i4++) {
            try {
    ...
    private boolean pullMoreData() throws IOException {
        this.buffer.compact();
        int i4 = this.bundleInputStream.read(this.buffer.array(), this.buffer.arrayOffset() + this.buffer.position(), this.buffer.remaining());
        boolean z4 = i4 > 0;
        if (z4) {
    ...
    private BundleElement readNextElement() {
        ...
        int i4 = Integer.parseInt(lengthPrefix);
        String jsonString = readJsonString(i4);
        this.bytesRead += (long) (lengthPrefix.getBytes(UTF8_CHARSET).length + i4);
