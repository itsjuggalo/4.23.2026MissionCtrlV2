...
public class SQLiteLruReferenceDelegate implements ReferenceDelegate, LruDelegate {
    ...
    public static /* synthetic */ void b(SQLiteLruReferenceDelegate sQLiteLruReferenceDelegate, int[] iArr, List list, ResourcePath[] resourcePathArr, Cursor cursor) {
        ...
        if (!sQLiteLruReferenceDelegate.isPinned(documentKeyFromPath)) {
            iArr[0] = iArr[0] + 1;
            list.add(documentKeyFromPath);
            sQLiteLruReferenceDelegate.removeSentinel(documentKeyFromPath);
        }
    ...
    public void forEachOrphanedDocumentSequenceNumber(final Consumer<Long> consumer) {
        this.persistence.query("select sequence_number from target_documents group by path having COUNT(*) = 1 AND target_id = 0").forEach(new Consumer() { // from class: com.google.firebase.firestore.local.Y
    ...
    public long getSequenceNumberCount() {
        return this.persistence.getTargetCache().getTargetCount() + ((Long) this.persistence.query("SELECT COUNT(*) FROM (SELECT sequence_number FROM target_documents GROUP BY path HAVING COUNT(*) = 1 AND target_id = 0)").firstValue(new Function() { // from class: com.google.firebase.firestore.local.a0
    ...
    public int removeOrphanedDocuments(long j4) {
        ...
        while (this.persistence.query("select path from target_documents group by path having COUNT(*) = 1 AND target_id = 0 AND sequence_number <= ? AND path > ? LIMIT ?").binding(Long.valueOf(j4), EncodedPath.encode(resourcePathArr[0]), 100).forEach(new Consumer() { // from class: com.google.firebase.firestore.local.Z
            @Override // com.google.firebase.firestore.util.Consumer
            public final void accept(Object obj) {
                SQLiteLruReferenceDelegate.b(this.f10558a, iArr, arrayList, resourcePathArr, (Cursor) obj);
            }
        }) == 100) {
        }
    ...
    public void start(long j4) {
        this.listenSequence = new ListenSequence(j4);
    }
