#!/usr/bin/env python3
"""portfolio_to_discord.py — scheduled positions + P&L snapshot → #portfolio-status.
...
def main():
    ...
    cid = sd.resolve_channel("portfolio-status")
    sd.post(cid, f"📸 SNAPSHOT · {stamp}\n{text}")
    print(f"[portfolio] posted snapshot to #portfolio-status @ {stamp}", flush=True)
